    ...
    public RelationshipDto toRelationshipDto(
        ...
        if (relationship.isInLaw()) {
            Integer relatedPersons = Optional
                    ...
                    .orElse(0);
            if (relatedPersons == 1) {
                Integer spouseId = relationship.relatedPersonIds().getFirst();
        ...
        return RelationshipDto.builder()
                ...
                .adoptionType(Optional.ofNullable(relationship.adoptionTypeAsc())
                        .map(adoptionTypeAsc -> Optional.ofNullable(relationship.adoptionTypeDesc())
                                .filter(adoptionTypeDesc -> adoptionTypeDesc.ordinal() < adoptionTypeAsc.ordinal())
                                .orElse(adoptionTypeAsc))
                        .or(() -> Optional.ofNullable(relationship.adoptionTypeDesc()))
                        .orElse(null))
                .spouseSex(spouseSex)
